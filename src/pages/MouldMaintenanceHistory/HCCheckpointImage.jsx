...
export default function HCCheckpointImages() {
  ...
  useEffect(() => {
    ...
    const url = import.meta.env.DEV
      ? `/api/MouldMaintenanceHistoryHC/HCCheckpointDetails/get-checkpoint-images`
      : `${BASE}/MouldMaintenanceHistoryHC/HCCheckpointDetails/get-checkpoint-images`;

    setErrorMsg("");
    setLoading(true);
    axios
      .get(url, {
        params: { mouldName, instance, _: Date.now() },
        headers: { "Cache-Control": "no-cache" },
      })
      .then((res) => {
        // HC API may follow either {status:200,data:[...]} or {success:true,data:[...]}
  ...
  return (
    <DashboardLayout>
      <div className="p-6">
        ...
        <h1 className="text-xl font-bold mb-6">HC Checkpoint Images</h1>

        <div className="bg-white p-6 rounded-xl shadow-md">
          <p className="text-sm mb-6">
            <b>Mould Name:</b> {mouldName} | <b className="ml-3">Instance:</b> {instance}
          </p>
              ...
              {images.map((img, index) => (
                ...
                >
                  ...
                  <p className="text-xs text-center mt-1 text-gray-500">
                    CP {img.checkpoint ?? img.Checkpoints ?? "-"}
                  </p>
                </div>
              ))}
            </div>
